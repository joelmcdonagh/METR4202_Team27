...
class WaypointCycler(Node):
    def __init__(self):
        ...
        #Create subscriber  to behavior_treelog topic
...
# keep track of number of wyapoints sent
        self.waypoint_counter = 0

#manually input two waypoints
        p0 = PoseStamped()
        ...
        def bt_log_callback(self, msg:BehaviorTreeLog):
            #check status of navigate recovery node 
            for event in msg.event_log:
                if event.node_name == 'NavigateRecovery' and event.current_status == 'IDLE':
                    self.send_waypoint()

    def send_waypoint(self):
        #keep tack of current waypoint number
        self.waypoint_counter += 1

        #mod operation to determine which way to send
...
def main(args=None):
    ...
    # create new waypointcycler node
